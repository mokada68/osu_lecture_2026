...
# 【修正箇所】R3（中継ルーター）自身に、PC-A側およびPC-B側へのルートを教える
# これにより、R3がパケットを受け取った際に正しく中継できるようになります
ip netns exec r3 ip route add 192.168.1.0/24 via 10.0.13.1
ip netns exec r3 ip route add 192.168.2.0/24 via 10.0.23.2
...
echo "--> 経路の構築が完了しました！（R3への中継ルートを追加しました）"
echo ""
